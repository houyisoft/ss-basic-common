...
func (r *Redis) Connect(ip string) bool {
	...
	//if _, err := r.conn.Do("AUTH", config.REDIS_PASSWORD); err != nil {
	if _, err := r.conn.Do("AUTH", nil); err != nil {
		r.conn.Close()
		return false
	}
	return true
...
func (r *Redis) LRange(key string, start int, stop int) []string {
	if !r.isConn() {
		return []string{}
	}
...
func (r *Redis) MultiDeleteList(index int, key string, value string) bool {
	if !r.isConn() {
		return false
	}
	r.conn.Send("multi")
	r.conn.Send("lset", key, index, value)
	r.conn.Send("lrem", key, index, value)
