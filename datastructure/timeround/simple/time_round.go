...
var timewheels [5][]*linkedlist.Node                                         //定义5个时间轮
var TimerMap map[string]*linkedlist.Node = make(map[string]*linkedlist.Node) //保存待执行的计时器，方便按链表节点指针地址直接删除定时器
...
func SetTimer(name string, inteval uint32, handler func(interface{}), args interface{}) {
	...
	for offset >= element_cnt_per_wheel[bucket_no] { //偏移量大于当前时间轮容量，则需要向高位进位
		...
		fmt.Println("bucket_no=%v",bucket_no)
		...
		fmt.Println("base_per_wheel",base_per_wheel[bucket_no])
		fmt.Println("element_cnt_per_wheel",element_cnt_per_wheel[bucket_no])
		fmt.Println("newest",newest[bucket_no])
		fmt.Println("tmp",tmp)
...
func step() {
	//var dolist list.List
	{
		...
		for bucket_no = 0; bucket_no < wheel_cnt; bucket_no++ {
			...
			//fmt.Println(newest)
			var head *linkedlist.Node = timewheels[bucket_no][newest[bucket_no]] //返回当前指针指向的槽位置的表头
			var firstElement *linkedlist.Node = head.Next()
			for firstElement != nil { //链表不为空
				if value, ok := firstElement.Data().(Timer); ok { //如果element里面确实存储了Timer类型的数值，那么ok返回true，否则返回false。
					...
					linkedlist.Delete(firstElement) //删除定时器
				}
...
func Delete(element *linkedlist.Node)  {
	linkedlist.Delete(element) //删除定时器
}
...
func init() { //初始化
	...
	for bucket_no = 0; bucket_no < wheel_cnt; bucket_no++ {
		...
		for ; i < element_cnt_per_wheel[bucket_no]; i++ {
			timewheels[bucket_no] = append(timewheels[bucket_no], new(linkedlist.Node))
		}
	}
}
